...
function checkResponseCode(response: AxiosResponse<any, any>) {
  if (response.status > 200 && response.status <= 300) {
    throw new Error(
      `An error occurred while fetching data from the server. ${response.statusText}. Status: ${response.status}. ${response.request?.url}`,
    );
  }
}
...
export async function getRandomImageFromApi(
  message: IMessage,
  api: NekosAPIAxiosProxy,
) {
